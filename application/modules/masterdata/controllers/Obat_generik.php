...
class Obat_generik extends MX_Controller {

	function __construct()
    {
        parent::__construct();
				$this->load->model('Obat_generik_model');
				$this->user_id = 1;
    }
	...
	public function store()
	{
			...
			$result = $this->Obat_generik_model->insert($data);
			if ($result)
			{
	...
	{
			$where['id'] = $id;
			$data = $this->Obat_generik_model->get_data_by($where)->row_array();
	...
	public function update($id)
	{
			...
			$result = $this->Obat_generik_model->update($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
	...
	public function delete($id)
	{
			...
			$result = $this->Obat_generik_model->delete($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
  ...
  {
      $list = $this->Obat_generik_model->get_datatables();
      $data = array();
      $no = $_POST['start'];
      ...
      $output = array(
                      "draw" => $_POST['draw'],
                      "recordsTotal" => $this->Obat_generik_model->count_all(),
                      "recordsFiltered" => $this->Obat_generik_model->count_filtered(),
                      "data" => $data,
              );
