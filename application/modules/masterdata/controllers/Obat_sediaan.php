...
class Obat_sediaan extends MX_Controller {

	function __construct()
    {
        parent::__construct();
				$this->load->model('Obat_sediaan_model');
				$this->user_id = 1;
    }

	public function index()
	{
			$data['page'] = 'masterdata/obat_sediaan/index';
			$data['title'] = 'Sediaan Obat';
			$data['role'] = '';
	...
	public function store()
	{
			...
			$result = $this->Obat_sediaan_model->insert($data);
			if ($result)
			{
	...
	{
			$where['id'] = $id;
			$data = $this->Obat_sediaan_model->get_data_by($where)->row_array();
	...
	public function update($id)
	{
			...
			$result = $this->Obat_sediaan_model->update($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
	...
	public function delete($id)
	{
			...
			$result = $this->Obat_sediaan_model->delete($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
  ...
  {
      $list = $this->Obat_sediaan_model->get_datatables();
      $data = array();
      $no = $_POST['start'];
      ...
      $output = array(
                      "draw" => $_POST['draw'],
                      "recordsTotal" => $this->Obat_sediaan_model->count_all(),
                      "recordsFiltered" => $this->Obat_sediaan_model->count_filtered(),
                      "data" => $data,
              );
