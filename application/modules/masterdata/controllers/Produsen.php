...
class Produsen extends MX_Controller {
	...
	{
			$data['page'] = 'masterdata/produsen/index';
			$data['title'] = 'Produsen Obat';
			$data['role'] = '';
	...
	public function store()
	{
			...
			$result = $this->Produsen_model->insert($data);
			if ($result)
			{
	...
	{
			$where['id'] = $id;
			$data = $this->Produsen_model->get_data_by($where)->row_array();
	...
	public function update($id)
	{
			...
			$result = $this->Produsen_model->update($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
	...
	public function delete($id)
	{
			...
			$result = $this->Produsen_model->delete($data, $id);
			if ($result)
			{
				$response['status'] = 'success';
  ...
  {
      $list = $this->Produsen_model->get_datatables();
      $data = array();
      $no = $_POST['start'];
      ...
      $output = array(
                      "draw" => $_POST['draw'],
                      "recordsTotal" => $this->Produsen_model->count_all(),
                      "recordsFiltered" => $this->Produsen_model->count_filtered(),
                      "data" => $data,
              );
